...
import { SegmentAnalysis as SegmentType, Customer } from '../types';
      ...
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
            ...
            客群洞察: {analysis.segmentName}
          </h2>
          <p className="text-sm text-slate-500">基于定义规则：{analysis.dsl}</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={onSave} className="flex items-center gap-1 px-3 py-1.5 bg-blue-50 text-blue-600 text-xs font-bold rounded-lg hover:bg-blue-100">
            <Save className="w-3 h-3" /> 保存客群
          </button>
          <button onClick={onViewAll} className="flex items-center gap-1 px-3 py-1.5 bg-slate-900 text-white text-xs font-bold rounded-lg hover:bg-slate-800">
